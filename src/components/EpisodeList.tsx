...
export default function EpisodeList({ addToFavorites, isFavorite, removeFromFavorites }: EpisodeListProps) {
  ...
  const filteredEpisodes = useMemo(() => {
    ...
    return search
      ? data.filter(ep =>
          ...
          `S${ep.season}E${ep.episode}`.toLowerCase().includes(search.toLowerCase())
        )
      : data;
  }, [data, search]);
  ...
  const currentEpisodes = filteredEpisodes.slice(startIndex, startIndex + itemsPerPage);
                  ...
                  <div className="card-content">
                    ...
                    <div className="card-actions">
                      <button
                        ...
                        aria-label={isFav ? `Remover ${episode.name} dos favoritos` : `Adicionar ${episode.name} aos favoritos`}
                      >
                        <span>{isFav ? '❤️' : '🤍'}</span>
